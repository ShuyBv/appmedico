...
<head>
    ...
    <title>Modificar Enfermera</title>
</head>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex items-center justify-center w-3/4 ml-auto">
            <div class="bg-white bg-opacity-10 p-8 md:p-10 rounded-lg shadow-xl w-full max-w-sm">
                <h2 class="text-3xl font-bold text-white text-center mb-6">Modificar Enfermera</h2>
                <form action="{{ route('enfermera.update', $enfermera->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4 flex items-center bg-white bg-opacity-20 rounded-md shadow-sm">
                        ...
                        <input type="text" name="nombre_completo" id="nombre_completo" value="{{$enfermera->nombre_completo }}"
                    ...
                    <div class="mb-4 flex items-center bg-white bg-opacity-20 rounded-md shadow-sm">
                        <img src="{{ asset('img/user.png') }}" alt="Sueldo Icon" class="w-6 h-6 ml-2">
                        <input type="number" name="sueldo" id="sueldo"  value="{{$enfermera->sueldo }}"  step="0.01"
                            class="flex-grow px-3 py-2 bg-transparent border-none rounded-md focus:outline-none focus:ring-0 text-white placeholder-white"
                            placeholder="Sueldo por consulta" required>
                    </div>
                    ...
                    <div class="flex-grow flex items-center justify-center mt-3">
                        <button type="button"
                            class="w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" onclick="location.href='/Enfermera'">
                            Regresar
                        </button>
                    </div>
                </form>
